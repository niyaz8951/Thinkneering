...
(function () {
  ...
  async function load() {
    ...
    try {
      ...
      if ($('guidance') && !$('guidance').dataset.touched) $('guidance').value = body.guidance || '';
      if ($('corrections-count')) {
        var n = body.corrections || 0;
        $('corrections-count').textContent = n + (n === 1 ? ' correction' : ' corrections');
      }
      render();
  ...
  function body(e) {
    ...
    return (e.context_seen
      ...
      // Headword and part of speech decide the node's title and lane on the
      // Dictionary map, so both can be corrected before approval.
      field(e.id, 'term', 'Headword (dictionary form, as it will be filed)', e.term, 1) +
      posField(e.id, e.pos) +
  ...
  var POS = ['noun', 'verb', 'adjective', 'adverb', 'phrase', 'idiom', 'other'];

  function posField(id, value) {
    var domId = 'f-' + id + '-pos';
    return '<label class="kg-field" style="margin-top:var(--space-3)">' +
      '<span class="kg-label" for="' + domId + '">Part of speech — decides the lane on the map</span>' +
      '<select class="kg-input" id="' + domId + '" data-field="pos">' +
        '<option value=""' + (!value ? ' selected' : '') + '>Not set (files under Nouns)</option>' +
        POS.map(function (p) {
          return '<option value="' + p + '"' + (value === p ? ' selected' : '') + '>' + p + '</option>';
        }).join('') +
      '</select></label>';
  }
  ...
  async function saveGuidance() {
    var box = $('guidance');
    $('guidance-status').textContent = 'Saving…';
    try {
      await api('/api/admin/dictionary', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ guidance: box.value })
      });
      delete box.dataset.touched;
      $('guidance-status').textContent = 'Saved. The model reads this on its next draft.';
    } catch (err) {
      $('guidance-status').textContent = err.message;
    }
  }

  document.addEventListener('DOMContentLoaded', function () {
    if ($('guidance')) {
      $('guidance').addEventListener('input', function () { $('guidance').dataset.touched = '1'; });
      $('guidance-save').addEventListener('click', saveGuidance);
    }
