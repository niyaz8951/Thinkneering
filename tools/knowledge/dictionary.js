...
(function () {
  ...
  function card(e) {
    ...
    var meta = [
      e.domain,
      'looked up ' + (e.lookup_count || 0) + '×',
      corrected ? corrected + ' marked “not quite”' : null,
      fmt(e.created_at)
    ].filter(Boolean).join(' · ');
    ...
    return '<article class="kg-card" data-id="' + esc(e.id) + '" style="margin-bottom:var(--space-3)">' +
      '<div class="kg-card-head">' +
        '<h3 class="kg-list-title">' + esc(e.term) + '</h3>' +
        '<span class="kg-pill kg-pill-' + esc(e.status === 'approved' ? 'approved' : 'draft') + '">' +
          esc(e.status) + '</span>' +
      '</div>' +
      '<p class="kg-card-meta">' + esc(meta) + '</p>' +
  ...
  function field(id, name, label, value, rows) {
    var domId = 'f-' + id + '-' + name;
    return '<label class="kg-field kg-field-grow" style="margin-top:var(--space-3)">' +
