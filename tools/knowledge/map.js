...
(function () {
  ...
  function rootCmp(a, b) {
    return (laneIndex(a.lane) - laneIndex(b.lane)) || (authoredPos(a).y - authoredPos(b).y) || a.title.localeCompare(b.title);
  }
  ...
  function renderOutline() {
    ...
    if (ol.focusId && currentView === 'outline' && !sheetOpen) focusLine(ol.focusId, ol.selectAfter);
  ...
  function lineHTML(n, t, depth, shown) {
    ...
    return '<div class="kg-ol-item kg-list-row' + (n.id === ol.focusId ? ' is-focus' : '') +
  ...
  async function outlineAdd(asChild) {
    ...
    if (ref && asChild) {
      ...
      lane = ref.lane; x = authoredPos(ref).x + 40;
      if (!prev) { next = null; }
    } else if (ref) {
      ...
      lane = ref.lane; x = authoredPos(ref).x;
    } else {
      var roots = t.children[''] || [];
      prev = roots[roots.length - 1] || null;
      if (prev) { lane = prev.lane; x = authoredPos(prev).x; }
    }

    var y = ref && asChild && !prev ? authoredPos(ref).y + ROW_H / 2 : yBetween(apos(prev), apos(next));
  ...
  async function reparent(n, t, newParentId, afterId) {
    ...
    try {
      ...
      var ay = prev || next ? yBetween(apos(prev), apos(next)) : authoredPos(ref).y + ROW_H / 2;
      var ax = newParentId ? (authoredPos(nodeById(newParentId)).x + 40) : (prev ? authoredPos(prev).x : authoredPos(n).x);
      setAuthored(n, ax, ay);
  ...
  async function outlineMove(dir) {
    ...
    var ny = b.y, oy = a.y;
    if (ny === oy) ny += dir;   // two nodes on one row: nudge so the order sticks
    setAuthored(n, a.x, ny); setAuthored(other, b.x, oy);
