/* =====================================================================
   ...
   Two views of that one store: the graph (canvas) and the outline. Both
   render from the same `nodes` and `edges`, both write through the same
   API, and reload() redraws both after every write.
   ===================================================================== */
(function () {
  'use strict';
  ...
  function treeRelation() {
    return (pack && pack.outline && pack.outline.relation) || 'contains';
  }
  ...
  function orderCmp(a, b) {
    return (a.y - b.y) || a.title.localeCompare(b.title);
  }

  function rootCmp(a, b) {
    return (laneIndex(a.lane) - laneIndex(b.lane)) || (a.y - b.y) || a.title.localeCompare(b.title);
  }
  ...
  function crossLinks(id) {
    ...
    return edges.filter(function (e) {
      return e.relation !== rel && (e.from === id || e.to === id);
    });
  }
  ...
  function renderOutline() {
    ...
    // Never redraw under a title that is mid-edit; the save that follows
    // the edit calls reload(), and that redraw carries the new text.
    if (ol.dirty && box.contains(document.activeElement)) return;
    ...
    if (ol.focusId && !$('view-outline').hidden && !sheetOpen) focusLine(ol.focusId, ol.selectAfter);
  ...
  function lineHTML(n, t, depth, shown) {
    ...
    return '<div class="kg-ol-item' + (n.id === ol.focusId ? ' is-focus' : '') +
      ...
      '<div class="kg-ol-line">' +
        ...
        '<span class="kg-ol-dot" style="background:' + colourOf(n) + '"' +
        ...
        '<span class="kg-ol-meta">' +
          ...
          '<button type="button" class="kg-icon-btn kg-ol-openbtn" data-ol-open="' + n.id +
            '" aria-label="Open ' + esc(n.title) + '" title="Open">' +
            '<svg class="kg-ico" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6"/></svg></button>' +
        '</span>' +
      '</div>' +
      (n.summary || chips
        ? '<div class="kg-ol-sub">' +
            (n.summary ? '<span class="kg-ol-summary">' + esc(n.summary) + '</span>' : '') +
            chips +
          '</div>'
        : '') +
      (kids.length && !folded
  ...
  async function savePosition(n) {
    return postNode({ id: n.id, positionOnly: true, x: n.x, y: n.y, lane: n.lane });
  }
  ...
  async function commitTitle(id, text) {
    ...
    try {
      var body = await postNode({
        id: n.id, kind: n.kind, title: title, lane: n.lane, summary: n.summary,
        aliases: n.aliases, body: n.body, attributes: n.attributes, tags: n.tags,
        standards: n.standards, x: n.x, y: n.y
      });
  ...
  async function changeKind(id, kind) {
    ...
    try {
      var body = await postNode({
        id: n.id, kind: kind, title: n.title, lane: n.lane, summary: n.summary,
        aliases: n.aliases, body: n.body, attributes: n.attributes, tags: n.tags,
        standards: n.standards, x: n.x, y: n.y
      });
      setStatus(n.status === 'approved' && body.status === 'proposed'
  ...
  async function outlineAdd(asChild) {
    ...
    var parentId = null, prev = null, next = null, lane = (lanes[0] || {}).id || '', x = laneX(0);

    if (ref && asChild) {
      parentId = ref.id;
      var kids = t.children[ref.id] || [];
      prev = kids[kids.length - 1] || null;
      lane = ref.lane; x = ref.x + 40;
      if (!prev) { next = null; }
    } else if (ref) {
      ...
      lane = ref.lane; x = ref.x;
    } else {
      var roots = t.children[''] || [];
      prev = roots[roots.length - 1] || null;
      if (prev) { lane = prev.lane; x = prev.x; }
    }

    var y = ref && asChild && !prev ? ref.y + ROW_H / 2 : yBetween(prev, next);
    ...
    try {
      ...
      if (parentId) await postEdge(parentId, body.id);
  ...
  async function reparent(n, t, newParentId, afterId) {
    ...
    setStatus('Moving…');
    try {
      if (old) await deleteEdge(old.id);
      if (newParentId) {
        await postEdge(newParentId, n.id);
      ...
      var ref = nodeById(newParentId) || prev || n;
      n.y = prev || next ? yBetween(prev, next) : ref.y + ROW_H / 2;
      n.x = newParentId ? (nodeById(newParentId).x + 40) : (prev ? prev.x : n.x);
  ...
  async function outlineMove(dir) {
    ...
    var other = sib[i + dir];
    if (!other) return;
    var y = n.y; n.y = other.y; other.y = y;
    if (n.y === other.y) n.y += dir;   // two nodes on one row: nudge so the order sticks
  ...
  function bindOutline() {
    ...
    box.addEventListener('focusin', function (ev) {
      var el = ev.target.closest ? ev.target.closest('[data-ol-title]') : null;
      if (el) setLineFocus(el.getAttribute('data-ol-title'));
    });
    ...
    box.addEventListener('focusout', function (ev) {
      var el = ev.target.closest ? ev.target.closest('[data-ol-title]') : null;
      if (el && ol.dirty) commitTitle(el.getAttribute('data-ol-title'), el.textContent);
    });
    ...
    box.addEventListener('click', function (ev) {
      ...
      var open = ev.target.closest('[data-ol-open]');
      if (open) { outlineOpen(open.getAttribute('data-ol-open')); return; }
    ...
    $('ol-add').addEventListener('click', function () { outlineAdd(false); });
    $('ol-child').addEventListener('click', function () { outlineAdd(true); });
    $('ol-indent').addEventListener('click', outlineIndent);
    $('ol-outdent').addEventListener('click', outlineOutdent);
    $('ol-up').addEventListener('click', function () { outlineMove(-1); });
    $('ol-down').addEventListener('click', function () { outlineMove(1); });
