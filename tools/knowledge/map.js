...
(function () {
  ...
  function renderOutline() {
    ...
    lanes.concat([{ id: '', label: 'Unassigned', token: '--color-text-muted' }]).forEach(function (lane) {
      ...
      group.forEach(function (n) {
        ...
        html += '<li class="kg-list-row' + (n.id === selectedId ? ' is-selected' : '') + '">' +
          '<span class="kg-list-bar" style="background:' + colourOf(n) + '"></span>' +
          '<div class="kg-list-body"><p class="kg-list-title">' +
          '<button type="button" class="kg-linkish" data-goto="' + n.id + '">' +
          esc(n.title) + '</button> ' +
          '<span class="kg-pill kg-pill-' + esc(n.status) + '">' + esc(n.status) + '</span>' +
          (openCount
            ? ' <span class="kg-pill kg-pill-open">' + openCount + ' open</span>'
            : '') +
          '</p>' +
  ...
  function bind() {
    ...
    $('outline').addEventListener('click', function (ev) {
      ...
      renderOutline();          // move the selected marker
      openSheet('details');
    });
