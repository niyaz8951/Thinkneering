...
(function () {
  ...
  async function loadMaps() {
    try {
      ...
      maps = body.maps || [];
      $('import-map').innerHTML = maps.map(function (m) {
  ...
  function switchTab(name) {
    ['queue', 'access', 'gaps', 'questions', 'import'].forEach(function (t) { $('tab-' + t).hidden = t !== name; });
    ...
    if (name === 'import' && !maps.length) loadMaps();
  }
  ...
  /* ── Import CSV ────────────────────────────────────────────────────
     Preview, read the numbers, then Apply. The endpoint refuses to delete
     and refuses to approve, so the worst an import can do is add drafts. */
  var importPlan = null;
  ...
  async function runImport(apply) {
    ...
    try {
      var body = await api('/api/admin/import', {
        ...
        body: JSON.stringify({
          mapId: $('import-map').value, table: $('import-table').value, csv: csv,
  ...
  function renderImport(body, applied) {
    var c = body.counts || {};
    var summary = Object.keys(c).map(function (k) { return c[k] + ' ' + k; }).join(' · ');
    var rows = (body.rows || []).slice(0, 400).map(function (r) {
      return '<tr><td class="kg-mono">' + r.line + '</td><td>' + esc(r.action) + '</td><td>' +
        esc(r.title || '') + '</td><td class="kg-muted">' + esc(r.reason || '') + '</td></tr>';
    }).join('');
    $('import-result').innerHTML =
      '<p class="kg-muted"><strong>' + (applied ? 'Applied' : 'Preview') + ':</strong> ' + esc(summary) + '</p>' +
      '<div class="kg-table-wrap"><table class="kg-table"><thead><tr><th>Line</th><th>Action</th><th>Title</th><th>Note</th></tr></thead><tbody>' +
      rows + '</tbody></table></div>' +
      ((body.rows || []).length > 400 ? '<p class="kg-muted">Showing the first 400 rows.</p>' : '');
  }
