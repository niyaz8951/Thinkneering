...
(function () {
    ...
    function next(bi) {
      if (bi >= batches.length) {
        ...
        checkConflicts(targetRows, product, factory).then(function (conf) {
          ...
          if (settledCount) {
            extra += ' ' + settledCount + ' row(s) were answered directly from the datasheet ' +
      ...
      fetch('/api/compliance/ai-suggest', {
        ...
        (d.suggestions || []).forEach(function (s, k) {
          ...
          gotAny = true;
          var row = batch[k].r;
          // A withdrawn answer is an UNANSWERED row, not a guess. It gets no
          // "[GUESS]" prefix (nothing was guessed — the answer was removed),
          // no amber "wild guess" styling, and no remark. The reason lives in
          // Comments, which is internal and deleted before sending.
          var isWithdrawn = s.guarded === true;
          var isGuess = !isWithdrawn && s.verified === false;
          row.compliance = (isGuess ? '[GUESS] ' : '') + s.status;
          row.remarks = isWithdrawn ? '' : s.remarks;
          ...
          row.auto = {
            type: isWithdrawn ? 'ai-blank' : (isGuess ? 'ai-guess' : 'ai'),
            score: 0, from: '', comments: s.comments || ''
          };
          suggested++;
        });
        guardedCount += (d.guarded || 0);
        settledCount += (d.settled || 0);
        if (!gotAny) {
  ...
  function previewComment(r) {
    ...
    if (r.auto.type === 'ai-blank') return r.auto.comments || 'AI could not answer this — left blank on purpose.';
  ...
  function renderPreview(rows) {
    ...
    rows.forEach(function (r) {
      ...
      if (r.auto && (r.compliance || r.remarks)) {
        kind = r.auto.type === 'ai-guess' ? 'guess'
             : r.auto.type === 'ai-blank' ? 'empty'
             : (r.auto.type === 'ai' ? 'ai' : 'lib');
        if (kind === 'guess') guessFilled++;
        else if (kind === 'ai') aiFilled++;
        else if (kind === 'empty') { /* withdrawn — counted as still blank */ }
        else libFilled++;
        if (kind === 'empty' && isBody) blanks++;
      } else if (isBody) {
        blanks++;
      }
      ...
      [r.compliance || '', r.remarks || ''].forEach(function (val) {
        ...
        // A withdrawn row is styled as EMPTY, not amber. Amber says "here is
        // an answer, check it"; there is no answer here.
        td.className = r.conflict ? 'col-conflict'
          : (kind === 'guess' ? 'col-guess'
          : (kind === 'ai' ? 'col-ai'
          : (kind === 'lib' ? 'col-auto' : 'col-empty')));
        td.textContent = val;
        if (r.conflict) {
          ...
          td.title = 'AI produced an answer that could not be traced to any source, so it was ' +
            'withdrawn. See the Comments column for what went wrong. Answer this row yourself.';
        }
        tr.appendChild(td);
      });
