...
(function () {
  ...
  function runAiSuggest() {
    ...
    if (!pf) return;
    var product = pf.product, factory = pf.factory;
    // Selection datasheet is REQUIRED — it is authorized source #1, checked
    // before the library, and it is what unlocks this button. The server
    // refuses the request without it too, so this is a friendly message
    // rather than the actual gate.
    if (!datasheetLoaded()) {
      setAiStatus('Upload the selection datasheet first — AI answers are checked against it.', 'error');
      return;
    }
    ...
    function next(bi) {
      if (bi >= batches.length) {
        ...
        checkConflicts(targetRows, product, factory).then(function (conf) {
          ...
          var extra = batchesWithNoUsableAnswers
            ? ' (' + batchesWithNoUsableAnswers + ' batch(es) returned no usable answer — the model ' +
              'may have declined those clauses; try again or rephrase the selection datasheet.' +
              (lastDebugSnippet ? ' What it actually said: "' + lastDebugSnippet + '"' : '') + ')'
            : '';
      ...
      fetch('/api/compliance/ai-suggest', {
        ...
        if (!gotAny) {
          ...
          if (d.debug) lastDebugSnippet = String(d.debug).slice(0, 180);
        }
        next(bi + 1);
      }).catch(function (err) {
  ...
  // The AI button now has FOUR conditions, all of which must hold, and they
  // are checked in exactly one place so the datasheet handler, renderPreview
  // and clearAll can never disagree about the button state:
  //   1. the account has AI access        2. a datasheet is loaded
  //   3. rows exist with blanks to fill   4. no extraction in flight
  ...
  function refreshAiButton() {
    ...
    btnAi.disabled = !aiEnabled || extractingDatasheet || !datasheetLoaded() || blanks === 0;
    ...
    else if (!datasheetLoaded()) hint.textContent = 'Upload the selection datasheet to enable AI.';
  ...
  function exportAnswerLog() {
    ...
    fetch('/api/compliance/answer-log?product=' + encodeURIComponent(product) + '&factory=' + encodeURIComponent(factory))
      .then(safeJson).then(function (d) {
        ...
        setExportLogStatus('Exported ' + rows.length + ' latest answers. Correct offline, then use it to update ' +
          libFileName(product, factory) + ' in the repo.', 'ok');
      }).catch(function (err) {
  ...
  function finishBuild(rows, product, factory, baseMsg) {
    ...
    // Convert-only users: parse + scope rule + highlighting only. No library,
    // no conflicts, no AI. One clean pass.
    if (basicMode) {
      var ruled0 = applyScopeRules(rows);
      renderPreview(rows);
      setStatus(baseMsg + (ruled0 ? ' · ' + ruled0 + ' By Contractor (scope rule)' : '') + '.', 'ok');
      return;
    }
  ...
  function onReadyStateChange() {
    ...
    refreshConvertState();
    if (btnExportLog) btnExportLog.disabled = !productChosen();
    if (productChosen()) {
  ...
  if (btnExportLog) {
    btnExportLog.disabled = !productChosen();
    btnExportLog.addEventListener('click', exportAnswerLog);
  }
