...
(function () {
  ...
  function extractEntriesFromSheet(aoa) {
    ...
    for (var r = 0; r < Math.min(aoa.length, 15); r++) {
      ...
      for (var c = 0; c < row.length; c++) {
        ...
        // The library carries a fourth column your team writes for
        // themselves — why an answer is phrased the way it is, what to watch
        // for, what the certificate actually covers. It was being discarded.
        else if (v === 'internal comments' || v === 'internal comment' ||
                 v === 'comments' || v === 'notes') idx.note = c;
      }
    ...
    for (var dr = headerRow + 1; dr < aoa.length; dr++) {
      ...
      var note = cols.note != null ? String(row2[cols.note] == null ? '' : row2[cols.note]).trim() : '';
      if (spec && (comp || rem)) {
        entries.push({ spec: spec, compliance: comp, remarks: rem, note: note });
      }
    }
    ...
    rows.forEach(function (r) {
      ...
      if (scored.length && scored[0].s >= FUZZY_THRESHOLD) {
        ...
        r._aiContext = scored.slice(0, 5)
          .map(function (x) {
            return {
              spec: x.e.spec, compliance: x.e.compliance, remarks: x.e.remarks,
              // The internal comment says WHY the answer reads the way it
              // does — the caveat, the certificate's real scope, what to
              // watch for. It is the difference between copying a past
              // remark and understanding it.
              note: x.e.note || ''
            };
          });
      }
  ...
  // The remark CONTINUES the Compliance cell rather than repeating it. The
  // status column already says "By Contractor"; saying it again wastes the
  // first half of the remark a consultant reads.
  var BY_CONTRACTOR_REMARK = 'Daikin scope is equipment supply only.';
  ...
  function selectSource(which) {
    activeSource = (which === 'pdf' || which === 'text' || which === 'ask') ? which : 'text';
    var pdf = activeSource === 'pdf';
    var ask = activeSource === 'ask';
    if (tabAsk) {
      tabAsk.setAttribute('aria-selected', ask ? 'true' : 'false');
      tabAsk.setAttribute('aria-pressed', ask ? 'true' : 'false');
    }
    if (paneAsk) paneAsk.hidden = !ask;
    // Ask has no document to convert, so the Convert button and the
    // formatting panel would only be confusing next to it.
    var fmt = document.getElementById('panel-format');
    if (fmt) fmt.hidden = ask;
    ...
    panePdf.hidden = pdf ? false : true;
    paneText.hidden = (activeSource === 'text') ? false : true;
  ...
  var appTier = '';           // '' until resolved, then 'member' | 'pro'
  var signedIn = false;
  ...
  //   appMode ('basic' | 'full')  — LAYOUT. 'basic' fused the panels for a
  //     signed-out visitor. The site is account-only now, so every caller
  //     resolves to 'full' and the basic branches below are unreachable.
  //     They are left in place rather than torn out mid-drop; removing them
  //     is a separate pass with its own testing.
  //   appTier ('member'|'pro') — CAPABILITY. The datasheet and AI extras
  //     need admin-granted access on top of an account.
  /* ======================================================================
  ...
  var btnAskClear = document.getElementById('btn-ask-clear');
  var chatHistory = [];
  ...
  function libraryContextFor(text, product, factory) {
    var lib = libCache[libCacheKey(product, factory)];
    if (!lib || !lib.entries.length) return [];
    var t = tokenSet(normSpec(text));
    return lib.entries
      .map(function (e) { return { e: e, s: jaccard(t, e.tokens) }; })
      .sort(function (a, b) { return b.s - a.s; })
      .slice(0, 4)
      .map(function (x) {
        return { spec: x.e.spec, compliance: x.e.compliance, remarks: x.e.remarks, note: x.e.note || '' };
      });
  }

  function askQuestion() {
    var pf = requireProduct(setChatStatus);
    if (!pf) return;
    var q = (chatInput.value || '').trim();
    if (!q) return;
    if (q.length > 400) {
      setChatStatus('Too long for a question — use Paste text for a specification.', 'error');
      return;
    }
    chatInput.value = '';
    chatAppend('<p class="chat-q">' + chatEsc(q) + '</p>');
    ...
    return fetch('/api/compliance/ask', {
      ...
      body: JSON.stringify({
        ...
        context: libraryContextFor(q, pf.product, pf.factory), history: chatHistory.slice(-8)
      })
    }).then(safeJson).then(function (d) {
      if (d.error) throw new Error(d.error);
      ...
      // The endpoint returns { status, remarks, answer }: `answer` is the
      ...
      chatAppend(
        '<div class="chat-a">' +
        '<div class="chat-a__body">' + chatEsc(prose) + '</div>' +
        '<div class="chat-a__meta">' +
          '<span class="chip chat-a__status" data-status="' +
      ...
      setChatStatus(err.message || 'Could not get an answer.', 'error');
    }).then(function () { btnAsk.disabled = false; });
  }

  if (btnAsk) btnAsk.addEventListener('click', askQuestion);
  if (btnAskClear) btnAskClear.addEventListener('click', function () {
    chatHistory = [];
    chatLog.innerHTML = '<p class="muted" id="chat-empty">Ask about one requirement at a time. ' +
      'Answers use the same knowledge and the same rules as a converted row.</p>';
    setChatStatus('');
  });
  if (chatInput) chatInput.addEventListener('keydown', function (ev) {
    // Enter sends, Shift+Enter is a newline — questions are one line here.
    if (ev.key === 'Enter' && !ev.shiftKey) { ev.preventDefault(); askQuestion(); }
  });
  if (tabAsk) tabAsk.addEventListener('click', function () { selectSource('ask'); });
  ...
  /* ---- product knowledge base: export, edit in Excel, upload back ------
     One sheet, one row per attribute, shaped the way an engineer would draw
     it: Section / Attribute / Default / Option 2 ... Moving a value into the
     Default column is the whole edit. Everything the AI knows about what we
     build lives here, so this is the file to review regularly.          */
  function exportKb() {
    var product = productInput.value;
    if (!product || !window.XLSX) { setExportLogStatus('Select a product first.', 'error'); return; }
    setExportLogStatus('Exporting the ' + product + ' knowledge base…');
    fetch('/api/compliance/kb?product=' + encodeURIComponent(product))
      .then(safeJson).then(function (d) {
        if (d.error) throw new Error(d.error);
        var rows = d.entries || [];
        if (!rows.length) {
          setExportLogStatus('Nothing recorded for ' + product + ' yet — upload a datasheet and the ' +
            'values are collected automatically.', 'error');
          return;
        }
        var widest = rows.reduce(function (m, e) { return Math.max(m, e.options.length); }, 1);
        var head = ['Section', 'Attribute', 'Definition', 'Unit', 'Better when', 'Class scale', 'Default'];
        for (var i = 1; i < widest; i++) head.push('Option ' + (i + 1));
        head.push('Times seen');
        var aoa = [head].concat(rows.map(function (e) {
          var row = [e.section, e.attribute, e.definition || '', e.unit || '', e.direction || '', e.scale || ''];
          for (var i = 0; i < widest; i++) row.push(e.options[i] || '');
          row.push(e.timesSeen);
          return row;
        }));
        var wb = window.XLSX.utils.book_new();
        window.XLSX.utils.book_append_sheet(wb, window.XLSX.utils.aoa_to_sheet(aoa), 'Knowledge base');
        window.XLSX.writeFile(wb, product.replace(/ /g, '-') + '-knowledge-base.xlsx');
        setExportLogStatus('Exported ' + rows.length + ' attribute(s). Move a value into the Default ' +
          'column and upload it back to change what we quote as standard.', 'ok');
      }).catch(function (err) {
        setExportLogStatus('Export failed — ' + (err.message || 'error'), 'error');
      });
  }

  function importKb(file) {
    var product = productInput.value;
    ...
    file.arrayBuffer().then(function (buf) {
      ...
      // Find the columns by NAME rather than position, so an inserted column
      // or a reordered export does not silently shift every value.
      var head = (aoa[0] || []).map(function (h) { return String(h || '').toLowerCase().trim(); });
      var col = function (name) { return head.indexOf(name); };
      var iSec = col('section'), iAttr = col('attribute'), iDef = col('definition');
      var iUnit = col('unit'), iDir = col('better when'), iScale = col('class scale');
      var iFirst = col('default');
      if (iAttr < 0 || iFirst < 0) throw new Error('Needs at least an "Attribute" and a "Default" column.');
      var iSeen = col('times seen');

      var rows = [];
      for (var r = 1; r < aoa.length; r++) {
        var line = aoa[r] || [];
        var attribute = String(line[iAttr] == null ? '' : line[iAttr]).trim();
        if (!attribute) continue;
        var options = [];
        for (var c = iFirst; c < line.length; c++) {
          if (c === iSeen) continue;
          var cell = String(line[c] == null ? '' : line[c]).trim();
          if (cell) options.push(cell);
        }
        if (!options.length) continue;
        rows.push({
          section: iSec >= 0 ? String(line[iSec] || '').trim() : 'Unit',
          attribute: attribute,
          definition: iDef >= 0 ? String(line[iDef] || '').trim() : '',
          unit: iUnit >= 0 ? String(line[iUnit] || '').trim() : '',
          direction: iDir >= 0 ? String(line[iDir] || '').trim() : '',
          scale: iScale >= 0 ? String(line[iScale] || '').trim() : '',
          options: options
        });
      }
      if (!rows.length) throw new Error('No rows with an attribute and a default value.');
      setExportLogStatus('Saving ' + rows.length + ' attribute(s)…');
      return fetch('/api/compliance/kb', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ product: product, rows: rows })
      }).then(safeJson);
    }).then(function (d) {
      if (!d) return;
      if (d.error) throw new Error(d.error);
      setExportLogStatus('Saved ' + d.saved + ' attribute(s). This is now what the AI quotes as standard.', 'ok');
    }).catch(function (err) {
      setExportLogStatus('Upload failed — ' + (err.message || 'could not read that file'), 'error');
    });
  }

  if (btnExportOpts) btnExportOpts.addEventListener('click', exportKb);
    ...
    file.arrayBuffer().then(function (buf) {
      ...
      if (trainResult) {
        ...
        trainResult.innerHTML =
          ...
          (d.blank ? '<span class="chip">' + d.blank + ' deliberately left blank</span>' : '') +
  ...
  function applyTier(info) {
    ...
    appTier = (info && info.tier) || 'member';
    maxSpecPages = (info && info.maxPages) || (signedIn ? 50 : 10);
    // Middleware redirects a signed-out visitor before this page is served,
    // so signedIn is true for anyone who gets here. Both lines stay keyed on
    // it rather than hardcoded, so the day the access model changes again
  ...
  fetch('/api/compliance/access').then(safeJson).then(function (d) {
    applyTier(d && d.tier ? d : { signedIn: true, tier: 'member', maxPages: 50 });
  }).catch(function () {
    // Probe failed (offline, function error). Fall back to the free tier —
    // conversion still works entirely in the browser.
    // The access call failed. Every visitor here is signed in — middleware
      // saw to that — so assume the member floor rather than a guest tier
      // that no longer exists and would lock the page down to nothing.
      applyTier({ signedIn: true, tier: 'member', maxPages: 50 });
  });
