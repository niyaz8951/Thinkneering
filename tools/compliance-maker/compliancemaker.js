...
(function () {
  ...
  function askQuestion() {
    ...
    return fetch('/api/compliance/ask', {
      ...
      chatAppend(
        '<div class="chat-a">' +
        '<div class="chat-a__status">' + chatEsc(a.status || 'TO VERIFY') + '</div>' +
        (a.remarks ? '<div class="chat-a__remark">' + chatEsc(a.remarks) + '</div>' : '') +
        '</div>' +
        (d.note ? '<p class="chat-note">' + chatEsc(d.note) + '</p>' : ''));
      chatHistory.push({ role: 'assistant', content: (a.status || '') + '. ' + (a.remarks || '') });
      setChatStatus('');
    });
    }).catch(function (err) {
