...
-- ── Ledger ───────────────────────────────────────────────────────────
-- Every migration records itself, including ones already applied and
-- backfilled. INSERT OR IGNORE makes that a no-op on a live database, and
-- it means the rule is uniform: if a file is in db/ and is not a helper,
-- it ends this way.
INSERT OR IGNORE INTO schema_migrations (name, applied_at, note)
VALUES ('2026-09-knowledge-lanes.sql', datetime('now'), 'self-recorded');
