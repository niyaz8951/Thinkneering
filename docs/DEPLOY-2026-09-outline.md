...
### Why two maps looked old — and the guard so it cannot recur
The outline code is one file for every map, so "Dictionary and SBU look
old, HVAC and Business look new" was never per-map: it was the browser
cache. The last two fix drops changed `map.js` and `knowledge.css` without
bumping their `?v`, and — worse — `assets/js/dictionary.js` had been
changed under `?v=1`, which `/assets/*` serves immutable for a year with a
cache-first worker in front: the Library never received the headword
panel at all. Fixed by bumping `map.js?v=12`, `knowledge.css?v=13`,
`dictionary.js?v=2`, and the worker to `tn-v2`. Also found and fixed:
`global.css?v=9` on one page and `domain-*.js?v=1` on the Knowledge index
(pages disagreeing on which file to load).

`_dev/asset-versions.mjs` now records a hash per versioned asset and
`npm test` fails when a file changes under an unchanged `?v`. After a
deliberate change: bump the `?v` in the markup, then
`node _dev/asset-versions.mjs --update`.

Hard-refresh once after this deploy (or reinstall the app) so the new
worker takes over; from then on it is automatic.
