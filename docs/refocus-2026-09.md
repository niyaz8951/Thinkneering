...
node _dev/tests/desk.test.mjs                # 27 checks — the desk and ask-your-graph
...
Below the ask box, two counts: nodes waiting for review, and open actions across every map
you can see. Counts rather than lists, deliberately — a desk that shows everything
outstanding is a desk nobody clears. `/api/knowledge/actions` with no `mapId` now answers
across all visible maps, because "what is on my plate" is a question about a person, not
about one map.
