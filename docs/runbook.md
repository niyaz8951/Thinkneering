...
## Two ways to break the whole site at once

Both fail the *Function* publish while the *assets* publish anyway. The result
is identical and misleading: the site serves HTML, every API is dead, there is
no sign-in — and no auth gate either, because `_middleware.js` is in the same
bundle. It looks up, and it is wide open.

**1. A binding naming a resource that does not exist.** Cloudflare validates
bindings at publish time. `VECTORIZE` pointing at an index you have not created
yet is enough. Optional bindings therefore ship commented out in
`wrangler.toml`; uncomment only after creating the resource.

**2. An import reaching outside `functions/`.**

**Nothing under `functions/` may import from outside `functions/`.**

Pages bundles that directory, and one unresolvable import fails the whole
build. This shipped once, with `eval.js` importing from `_dev/`.

`npm test` now catches both — an escaping import, and an active binding for a
resource that may not exist.

**Always read the build log after a push.** `✨ Compiled Worker successfully`
is not success; the line that matters is the last one. A publish failure appears
*after* `Success: Assets published!`, which is exactly why the site looks
...
**`_redirects` destinations must be extensionless.** Pages strips `.html` with
its own 301, so a rewrite to `/section.html` fires that redirect, the address
bar becomes `/section`, and the original `/s/<slug>` is lost.

**Anchor every path regex.** `/^\/s\/?/` also matches `/section` and leaves
`ection`. That is a wrong answer rather than an error, which sends you looking
in the data instead of the routing. Use `/^\/s(?:\/(.*))?$/`.
...
build failed. Open `/api/catalog` directly — if it errors, that is confirmation.
Check the Cloudflare Pages build log, and run `npm test`, which checks for the
import rule above.
