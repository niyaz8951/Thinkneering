...
## The rule that breaks everything at once

**Nothing under `functions/` may import from outside `functions/`.**

Cloudflare Pages bundles that directory. An import reaching outside it cannot be
resolved at build time, and **one** unresolvable import fails the *whole*
Functions build. Every `/api/` route then errors together — including
`/api/catalog`, which builds the header — so the site renders as an empty page
with no nav, and nothing about the symptom points at the cause.

This shipped once, with `eval.js` importing its case set from `_dev/`. The fix
was moving the file into `functions/_lib/`.

`_dev/tests/engine.test.mjs` now fails on any escaping import, so `npm test`
catches it before a push does.
