...
(function () {
  ...
  // ------------------------------------------------------------- app
  // Installable as a phone app. The manifest link is added here rather than
  // to every page's <head>, so one file keeps every page installable; the
  // worker caches the static shell only (see /sw.js).
  function installApp() {
    var head = document.head;
    if (!el('link[rel="manifest"]')) {
      var m = document.createElement('link');
      m.rel = 'manifest'; m.href = '/manifest.webmanifest';
      head.appendChild(m);
    }
    if (!el('link[rel="apple-touch-icon"]')) {
      var a = document.createElement('link');
      a.rel = 'apple-touch-icon'; a.href = '/assets/icons/apple-touch-icon.png';
      head.appendChild(a);
    }
    if (!el('meta[name="theme-color"]')) {
      var t = document.createElement('meta');
      t.name = 'theme-color';
      t.content = getComputedStyle(document.documentElement).getPropertyValue('--color-primary').trim() || '#2f5fff';
      head.appendChild(t);
    }
