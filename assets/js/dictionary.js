...
(function () {
  ...
  function lookup(term, domain, sentence) {
    ...
    readJson('/api/dictionary/lookup', {
      ...
      .then(function (result) {
        ...
        if (result.payload && result.payload.term) {
          cache.set(domain + '::' + result.payload.term.toLowerCase(), result.payload);
        }
  ...
  function renderEntry(entry) {
    ...
    // The heading shows the dictionary form — "Run" for a reader who
    // selected "running" — with the selected form and the part of speech
    // beneath it, so the reader can see both the entry they got and why.
    var term = panel.querySelector('#tn-dict-term');
    if (entry.term) term.textContent = entry.term;
    term.classList.toggle('is-verified', verified);
    term.title = verified ? 'Reviewed and approved' : '';

    var meta = [];
    if (entry.pos) meta.push(entry.pos);
    if (entry.lookedUp && entry.term &&
        entry.lookedUp.toLowerCase() !== entry.term.toLowerCase()) {
      meta.push('looked up as \u201c' + entry.lookedUp + '\u201d');
    }
    meta.push(domainLabel(entry.domain || (current && current.domain)));
    panel.querySelector('#tn-dict-domain').textContent = meta.join(' \u00b7 ');
