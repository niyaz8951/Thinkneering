...
(function () {
  ...
  // A grant only ever changes the answer for some rows. canAccess() in
  // functions/_lib.js returns early for public+free and for auth+free, so it
  // never reaches the grant check on those. Listing them anyway gave a modal
  // of twenty buttons where a dozen did nothing at all.
  //
  // What matters is the EFFECTIVE level, not the row's own: a public
  // subsection under a restricted parent is restricted in practice, and an
  // item inherits from the section it sits in.
  var ACCESS_RANK = { public: 0, auth: 1, restricted: 2 };
  var PLAN_RANK = { free: 0, member: 1, pro: 2 };

  function strictestLevel(a, b) {
    return (ACCESS_RANK[a] || 0) >= (ACCESS_RANK[b] || 0) ? (a || 'public') : (b || 'public');
  }
  function strictestPlan(a, b) {
    return (PLAN_RANK[a] || 0) >= (PLAN_RANK[b] || 0) ? (a || 'free') : (b || 'free');
  }

  function effectiveAccess(sections, section, ownLevel, ownPlan) {
    var level = ownLevel || 'public';
    var plan = ownPlan || 'free';
    var cur = section;
    var guard = 0;                      // a mis-parented row must not spin here
    while (cur && guard++ < 12) {
      level = strictestLevel(level, cur.access_level);
      plan = strictestPlan(plan, cur.required_plan);
      if (!cur.parent_id) break;
      cur = sections.filter(function (x) { return x.id === cur.parent_id; })[0];
    }
    return { level: level, plan: plan };
  }

  function grantMatters(eff) {
    return eff.level === 'restricted' || eff.plan !== 'free';
  }
  ...
  async function editGrants(userId) {
    ...
    var sections = state.catalog.sections;
    var byId = {};
    sections.forEach(function (s) { byId[s.id] = s; });

    var all = []
      .concat(sections.map(function (s) {
        return { t: 'section', id: s.id, label: 'Section — ' + s.title,
                 eff: effectiveAccess(sections, s, s.access_level, s.required_plan) };
      }))
      .concat(state.catalog.items.map(function (i) {
        return { t: 'item', id: i.id, label: 'Item — ' + i.title,
                 eff: effectiveAccess(sections, byId[i.section_id], i.access_level, i.required_plan) };
      }))
      .concat(books.map(function (b) {
        return { t: 'book', id: b.id, label: 'Book — ' + b.title,
                 eff: effectiveAccess(sections, null, b.access_level, b.required_plan) };
      }));

    // Anything already granted stays visible even if it no longer matters,
    // so a stale grant can still be found and taken away.
    var useful = all.filter(function (r) { return grantMatters(r.eff) || have.has(key(r.t, r.id)); });
    var rows = state.grantsShowAll ? all : useful;
    var hidden = all.length - useful.length;
    ...
    openModal('Unlock content for this person',
      ...
      (hidden > 0
        ? '<p class="hint">' + hidden + ' of ' + all.length + ' entries are open to everyone already, ' +
          'so a grant on them would do nothing. ' +
          '<button class="btn btn--sm btn--quiet" data-grant-showall="' +
          (state.grantsShowAll ? 'off' : 'on') + '">' +
          (state.grantsShowAll ? 'Hide them' : 'Show them anyway') + '</button></p>'
        : '') +
      '<div class="table-wrap" style="max-height:340px;overflow:auto"><table class="data"><tbody>' +
      rows.map(function (r) {
        var on = have.has(key(r.t, r.id));
        var idle = !grantMatters(r.eff);
        return '<tr><td>' + esc(r.label) +
          '<br><span class="hint">' + esc(r.eff.level) + ' &middot; plan ' + esc(r.eff.plan) +
          (idle ? ' &middot; a grant changes nothing here' : '') + '</span></td>' +
    ...
    el('#modal-body').onclick = async function (ev) {
      var toggle = ev.target.closest('[data-grant-showall]');
      if (toggle) {
        state.grantsShowAll = toggle.getAttribute('data-grant-showall') === 'on';
        modal.close();
        editGrants(userId);
        return;
      }
  ...
  function itemForm(i, sectionId, books) {
    ...
    return field('Title', '<input name="title" type="text" value="' + esc(i.title || '') + '">') +
      ...
      field('Link', '<input name="href" type="text" value="' + esc(i.href || '') + '">', 'For tools, pages and links, e.g. /tools/compliance-maker/') +
