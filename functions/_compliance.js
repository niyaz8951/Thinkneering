...
// Kept as the floor for an account with no plan. It was the signed-out
// allowance; with no signed-out tier left, it is what a bare 'free' plan
// gets until an admin raises it.
...
//   member — signed in: convert, highlight, library pre-fill, conflicts, log
//   pro    — member + the `ai-review` item: + datasheet, AI suggestions
//
// The guest tier is gone with the free layer. Middleware redirects a
// signed-out visitor before any of this runs, so reaching here without a
// user means the session ended mid-request. That is a 401, not a downgrade
// to a tier the site no longer offers.
export async function complianceTier(context) {
  const user = await requireUser(context);
  if (!user) return { signedIn: false, tier: null, maxPages: 0, ai: false, signedOut: true };
