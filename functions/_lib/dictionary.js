...
/* Which lane a freshly approved word lands in. The lookup records the part
   of speech the model reported (dictionary_entries.pos), and that is what
   decides the lane on a word map: a verb goes to Verbs, an adjective to
   Adjectives & adverbs, a phrase to Phrases & idioms. Only a word with no
   recorded part of speech falls back to Nouns. An HVAC or business
   dictionary has one lane for terms. */
const POS_LANE = {
  noun: 'nouns',
  verb: 'verbs',
...
export function laneFor(domain, pos) {
  if (!isWordMap(domain)) return 'terms';
  return POS_LANE[String(pos || '').toLowerCase()] || 'nouns';
}

/* An English phrase or idiom is its own node kind in the word pack; every
   other word is a `word`. An HVAC term stays a `term`. */
export function kindFor(domain, pos) {
  if (!isWordMap(domain)) return 'term';
  if (pos === 'phrase') return 'phrase';
  if (pos === 'idiom') return 'idiom';
  return 'word';
}
...
export async function promoteToGraph(env, entry, uid) {
  ...
  // The forms readers selected ("running", "runs") index the node too, so a
  // graph search or a Compliance Maker clause using the inflected form still
  // lands on Run.
  const forms = parseOr(entry.forms_json, []);
  ...
  if (entry.node_id) {
    ...
    if (existing) {
      // Re-approving keeps the node where a person may have moved it,
      // unless the row now carries a part of speech the node was filed
      // without — then the lane follows the part of speech.
      const lane = entry.pos ? laneFor(entry.domain, entry.pos) : existing.lane;
      await env.DB.prepare(
        'UPDATE knowledge_nodes SET title = ?, aliases = ?, summary = ?, body = ?, tags = ?, ' +
        "lane = ?, status = 'approved', updated_by = ?, updated_at = ?, approved_by = ?, " +
        'approved_at = ?, version = version + 1 WHERE id = ?'
      ).bind(
        entry.term, jsonField(aliases), entry.meaning || '', body, jsonField(tags),
        lane, uid, now, uid, now, entry.node_id
      ).run();
  ...
  const lane = laneFor(entry.domain, entry.pos);
  const position = await nextPosition(env, mapId, lane);
  const kind = kindFor(entry.domain, entry.pos);
