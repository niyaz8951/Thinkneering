...
/**
 * Finalise a run of text nodes.
 *
 * Decode first, collapse second, and the order is load-bearing: `&nbsp;`
 * becomes a space that the collapse can then fold into its neighbours. The
 * other way round leaves a non-breaking space sitting in the middle of a
 * sentence, which survives every later cleanup because it is not whitespace to
 * anything that only trims.
 *
 * Applied to text nodes only. HTMLRewriter already decodes attribute values,
 * so running this over those too would decode them twice.
 */
...
export async function harvest(html, baseUrl) {
  ...
  function flushBlock() {
    if (!openBlock) return;
    const text = collapseText(openBlock.text);
        ...
        try {
          el.onEndTag(() => {
            ...
            if (tag === 'a') linkDepth = Math.max(0, linkDepth - 1);
              ...
              if (cell) cell.text = collapseText(cell.text);
              cell = null;
            } else if (
              ...
              if (tag === 'figcaption' && openBlock) {
                const captionText = collapseText(openBlock.text);
  ...
  // Accumulated from <title> text chunks, not an attribute, so it needs the
  // same decoding. This one becomes the book name on the shelf.
  meta.title = collapseText(meta.title);
