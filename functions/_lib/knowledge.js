...
/* ── Error handling ────────────────────────────────────────────────────────
   An exception thrown out of a Pages Function is served as Cloudflare's HTML
   error page. The front end then tries to JSON.parse it and reports
   "Unexpected token '<'", which tells the user nothing about what actually
   went wrong. Every handler is wrapped so a failure comes back as JSON with
...
export function schemaHint(err) {
  ...
  // Name the migration rather than saying "run the migrations" — there are
  // several, and the missing object says which one is outstanding.
  const file = /lanes/i.test(msg)
    ? 'db/2026-09-knowledge-lanes.sql'
    : 'db/2026-08-knowledge-graph.sql';
  return 'The database is missing part of the knowledge schema (' + msg + '). ' +
    'Run: npx wrangler d1 execute thinkneering-db --remote --file=./' + file +
    ' — the Cloudflare "Retry deployment" button does not run SQL.';
}
...
export function withJson(handler) {
  return async (context) => {
    try {
      ...
      if (isMissingSchema(err)) return json({ error: schemaHint(err), setup: true }, 503);
      return json({ error: String((err && err.message) || 'Unexpected server error') }, 500);
    }
  };
}
