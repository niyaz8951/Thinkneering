...
const PUBLIC_PATHS = [
  '/login/',
  '/signup/',
  // The browser fetches the manifest and registers the worker from whatever
  // page it is on — which, for a signed-out visitor, is the sign-in page.
  // Gating these would redirect both to HTML: the manifest would fail to
  // parse, the worker would fail to register, and the site would simply never
  // be installable. Neither file contains anything account-specific.
  '/manifest.webmanifest',
  '/sw.js',
  // Cached and served by the worker while offline, so it has to be reachable
  // without a session. It deliberately holds nothing personal.
  '/offline.html',
];
