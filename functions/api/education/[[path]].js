...
// PATCH  /api/education/book/<slug>  -> move it to a shelf       (admin)
...
export const onRequestPost = async ({ env, request, params, data }) => {
  ...
  await env.BOOKS.put(bookKey(slug, format), body, {
    ...
    customMetadata: {
      ...
      // Free-text shelf label. The library groups on it, so an empty value is
      // a real state — "Unsorted" — rather than a missing one to default away.
      collection: encodeMeta(dec(h.get('X-Book-Collection'))),
...
// --------------------------------------------------------------- shelve
// Move a book to a shelf after the fact.
//
// R2 has no way to edit an object's metadata in place: the only way to change
// customMetadata is to write the object again. So this reads the body back and
// re-puts it with the same bytes and one field changed.
//
// That makes the operation as expensive as the file is large, which is why it
// is a separate endpoint rather than folded into upload — and why it reads the
// object BEFORE deciding anything, so a failed read cannot leave a book with
// half its metadata rewritten.
export const onRequestPatch = async ({ env, request, params, data }) => {
  const parts = Array.isArray(params.path) ? params.path : [params.path].filter(Boolean);
  if (parts[0] !== 'book' || !parts[1]) return bad('Unknown Education request.', 404);

  const user = data.user;
  if (!educationCanManage(user)) return bad('Only an admin can move books.', user ? 403 : 401);
  ...
  if (!body) return bad('Send a JSON body.', 400);

  const book = (await listBooks(env)).find((b) => b.slug === slug);
  if (!book) return bad('That book is not in the library.', 404);

  // An empty shelf is a real value — it files the book under Unsorted, which
  // is how you take something off a shelf without inventing an "unfile" verb.
  const collection = String(body.collection === undefined ? book.collection : body.collection)
    .trim().slice(0, 60);

  if (collection === (book.collection || '')) {
    return json({ ok: true, changed: false, collection });
  }

  const key = bookKey(book.slug, book.format);
  const obj = await env.BOOKS.get(key);
  if (!obj) return bad('That book file is missing from the bucket.', 404);

  const bytes = await obj.arrayBuffer();

  // Everything else is carried across verbatim. Rebuilding the metadata from
  // the listing instead would quietly drop any field listBooks does not
  // surface — uploadedBy and uploadedAt among them.
  const meta = Object.assign({}, obj.customMetadata || {});
  meta.collection = encodeMeta(collection);
  meta.shelvedBy = user.email || user.id;
  meta.shelvedAt = new Date().toISOString();

  await env.BOOKS.put(key, bytes, {
    httpMetadata: obj.httpMetadata || { contentType: FORMATS[book.format].mime },
    customMetadata: meta
  });

  await audit(env, user, 'book.shelve', slug,
              { from: book.collection || '(unsorted)', to: collection || '(unsorted)' });

  return json({ ok: true, changed: true, collection });
};
