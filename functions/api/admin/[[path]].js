...
import { PRODUCT_FACTORIES, isValidFactory, rebuildSections, isMissingTable, SETUP_HINT, normSectionName, PRODUCTS } from '../../_compliance.js';
...
export const onRequestGet = async ({ env, params, request, data }) => {
  ...
  switch (resource) {
    ...
    case 'compliance': {
      ...
      // Recent corrections: where a human overruled the AI. The single most
      // useful screen for spotting a fact that is wrong or missing, since a
      // repeated correction is usually one bad assumption showing up again
      // and again.
      let feedback = [];
      try {
        ({ results: feedback } = await DB.prepare(
          `SELECT spec_text, ai_status, ai_remarks, final_status, final_remarks, created_at
             FROM compliance_feedback
            WHERE product=?1 AND factory=?2 AND verdict='corrected'
            ORDER BY id DESC LIMIT 40`
        ).bind(product, factory).all());
      } catch { feedback = []; }

      let tally = { accepted: 0, corrected: 0, new: 0 };
      try {
        const { results } = await DB.prepare(
          `SELECT verdict, COUNT(*) AS n FROM compliance_feedback
            WHERE product=?1 AND factory=?2 GROUP BY verdict`
        ).bind(product, factory).all();
      ...
      let terms = [];
      try {
        ({ results: terms } = await DB.prepare(
          `SELECT * FROM compliance_terms WHERE product = ?1
            ORDER BY status='blocked', (status='new') DESC, times_seen DESC LIMIT 120`
        ).bind(product).all());
      } catch { terms = []; }

      // Specification-side classification: what kind of question each
      // section of a spec asks. Learned from confirmed answers.
      let specTopics = [];
      try {
        ({ results: specTopics } = await DB.prepare(
          `SELECT * FROM compliance_tree WHERE product = ?1 AND kind = 'spec-topic'
            ORDER BY status='blocked', times_seen DESC LIMIT 200`
        ).bind(product).all());
      } catch { specTopics = []; }

      let criteria = [];
      try {
        ({ results: criteria } = await DB.prepare(
          `SELECT * FROM compliance_criteria WHERE product = ?1 ORDER BY name LIMIT 200`
        ).bind(product).all());
      } catch { criteria = []; }

      return json({
        criteria: criteria || [],
        product, factory, pairs: PRODUCT_FACTORIES, terms: terms || [],
        facts: facts || [], sections: sections || [],
        feedback: feedback || [], tally, unitSections: unitSections || [],
        specTopics: specTopics || []
      });
    }
...
async function run(env, actor, body, op, ts) {
  ...
  switch (op) {
    ...
    case 'compliance.sections.rebuild': {
      ...
      let n = 0;
      try {
        n = await rebuildSections({ env }, product, factory, actor.id);
      } catch (err) {
        if (!isMissingTable(err)) throw err;
        return bad(SETUP_HINT, 503);
      }
      await log('sections.rebuild', { product, factory, sections: n });
      return json({ ok: true, sections: n });
    }

    case 'compliance.term.save': {
      const term = String(body.term || '').trim();
      if (!term) return bad('A term needs a name.');
      // Parse the range once, on save, so the plausibility check never has to
      // interpret free text at request time.
      const rng = String(body.typical_range || '').match(/(\d+(?:\.\d+)?)\s*(?:-|to|–)\s*(\d+(?:\.\d+)?)\s*([a-z%°]+)/i);
      await DB.prepare(
        `UPDATE compliance_terms
            SET term=?2, term_norm=?3, kind=?4, definition=?5, aliases=?6,
                typical_range=?7, range_min=?8, range_max=?9, range_unit=?10,
                status=?11, source=?12, updated_at=?13
          WHERE id=?1`
      ).bind(body.id, term, normSectionName(term), body.kind || 'component',
        String(body.definition || '').trim(), String(body.aliases || '').trim(),
        String(body.typical_range || '').trim(),
        rng ? parseFloat(rng[1]) : null, rng ? parseFloat(rng[2]) : null,
        rng ? rng[3].toLowerCase() : '', body.status || 'trusted',
        body.source || 'admin', ts).run();
      await log('term:' + body.id, { term, status: body.status });
      return json({ ok: true });
    }

    case 'compliance.term.delete': {
      await DB.prepare('DELETE FROM compliance_terms WHERE id=?1').bind(body.id).run();
      await log('term:' + body.id);
      return json({ ok: true });
    }

    // Describe undescribed terms from the model's own general knowledge.
    //
    // Everything written here lands as DRAFT, never trusted, and is marked
    // source='model'. This is general industry knowledge, not this factory's
    // data — useful for recognising words and spotting odd values, worthless
    // as an authority on what a specific unit is. A human decides which of it
    // is true here.
    case 'compliance.terms.describe': {
      if (!env.AI) return bad('Workers AI binding "AI" is not configured.', 500);
      const product = String(body.product || '').trim();
      if (!PRODUCTS.includes(product)) return bad('Pick a valid product.');

      const { results } = await DB.prepare(
        `SELECT id, term FROM compliance_terms
          WHERE product=?1 AND status='new' AND definition=''
          ORDER BY times_seen DESC LIMIT 10`
      ).bind(product).all();
      if (!(results || []).length) return json({ ok: true, described: 0 });
      ...
      for (const row of results) {
        const out = await env.AI.run('@cf/meta/llama-3.1-8b-instruct-fast', {
          messages: [
            { role: 'system', content:
              'You define HVAC terminology for engineers working on ' + product + ' equipment. ' +
              'Reply as JSON only: {"kind":"section|component|property|standard","definition":"one sentence, ' +
              'under 25 words","aliases":"other names for the same thing, comma separated, may be empty",' +
              '"typical_range":"typical market range with units if this is a measurable property, else empty"}. ' +
              'If you do not know the term, use an empty definition rather than guessing.' },
        ...
        let parsed = null;
        try {
          const text = String((out && (out.response || out.result)) || '').replace(/```(?:json)?/gi, '').trim();
          parsed = JSON.parse(text.slice(text.indexOf('{'), text.lastIndexOf('}') + 1));
        } catch { parsed = null; }
        if (!parsed || !String(parsed.definition || '').trim()) continue;

        const rng = String(parsed.typical_range || '').match(/(\d+(?:\.\d+)?)\s*(?:-|to|–)\s*(\d+(?:\.\d+)?)\s*([a-z%°]+)/i);
        await DB.prepare(
          `UPDATE compliance_terms SET kind=?2, definition=?3, aliases=?4, typical_range=?5,
                  range_min=?6, range_max=?7, range_unit=?8, status='draft', source='model', updated_at=?9
            WHERE id=?1`
        ).bind(row.id, parsed.kind || 'component', String(parsed.definition).slice(0, 300),
          String(parsed.aliases || '').slice(0, 200), String(parsed.typical_range || '').slice(0, 60),
          rng ? parseFloat(rng[1]) : null, rng ? parseFloat(rng[2]) : null,
          rng ? rng[3].toLowerCase() : '', ts).run();
        done++;
      }
      await log('terms.describe', { product, described: done });
      return json({ ok: true, described: done });
    }

    case 'compliance.criterion.save': {
      const name = String(body.name || '').trim();
      if (!name) return bad('A comparison rule needs a name.');
      const rid = body.id || ('ccrit_' + id('x').slice(-10));
      await DB.prepare(
        `INSERT INTO compliance_criteria
           (id,product,name,name_norm,match_terms,direction,unit,scale_order,notes,status,updated_at)
         VALUES (?1,?2,?3,?4,?5,?6,?7,?8,?9,'trusted',?10)
         ON CONFLICT(id) DO UPDATE SET
           name=?3, name_norm=?4, match_terms=?5, direction=?6, unit=?7,
           scale_order=?8, notes=?9, updated_at=?10`
      ).bind(rid, String(body.product || '').trim(), name, name.toLowerCase().trim(),
             String(body.terms || '').trim(), body.direction || 'unknown',
             String(body.unit || '').trim().toLowerCase(), String(body.scale || '').trim(),
             String(body.notes || '').trim(), ts).run();
      await log('criterion:' + rid, { name, direction: body.direction });
      return json({ ok: true, id: rid });
    }

    case 'compliance.criterion.delete': {
      await DB.prepare('DELETE FROM compliance_criteria WHERE id=?1').bind(body.id).run();
      await log('criterion:' + body.id);
      return json({ ok: true });
    }

    case 'compliance.topic.save': {
      // An admin edit moves the node off 'draft', which is what stops a
      // later rollup reclassifying it from the statistics.
      await DB.prepare(
        `UPDATE compliance_tree SET scope=?2, notes=?3, status=?4, updated_at=?5 WHERE id=?1`
      ).bind(body.id, body.scope || 'product', String(body.notes || '').trim(),
             body.status || 'trusted', ts).run();
      await log('topic:' + body.id, { scope: body.scope });
      return json({ ok: true });
    }

    case 'compliance.topic.delete': {
      await DB.prepare('DELETE FROM compliance_tree WHERE id=?1').bind(body.id).run();
      await log('topic:' + body.id);
      return json({ ok: true });
    }
