...
async function handleUsage(context) {
  ...
  // Feedback belongs to the entry that was shown, which is keyed by its
  // headword — "Run" — not by the form the reader selected. Without this the
  // review console's "flagged" count never lined up with the row.
  let key = termKey;
  const entryId = Number.isInteger(body.entryId) ? body.entryId : null;
  if (entryId) {
    const row = await env.DB.prepare('SELECT term_key FROM dictionary_entries WHERE id = ?1')
      .bind(entryId).first();
    if (row && row.term_key) key = row.term_key;
  }

  // Eight columns, eight values. The previous statement named seven columns
  // and bound eight values, so every "Was this useful?" tap failed on D1.
  await env.DB.prepare(
    `INSERT INTO dictionary_lookups (term_key, domain, entry_id, outcome, note, page_path, book_slug, user_email)
