...
async function handleLookup(context) {
  ...
  const headword = pickHeadword(generated.headword, term);
  ...
  if (existing) {
    ...
    await logOutcome(env, termKey, domain, existing.id, 'served', userEmail, request, book);
 ...
 */
export function pickHeadword(candidate, term) {
