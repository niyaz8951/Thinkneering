...
async function handlePrefetch(context) {
  ...
  // A chapter says "running"; the row is "Run" with "running" among its
  // forms. Match the remaining keys against the forms on file so the tap
  // opens instantly for an inflected word too. Keyed by the form, so the
  // client cache line the reader's tap will look for is the one filled.
  const missing = keys.filter((k) => !covered.has(k));
  if (missing.length) {
    try {
      const withForms = await env.DB.prepare(
        `SELECT * FROM dictionary_entries
         WHERE domain = ?1 AND status = 'approved' AND forms_json IS NOT NULL`
      ).bind(domain).all();
      for (const row of ((withForms && withForms.results) || [])) {
        let forms = [];
        try { forms = JSON.parse(row.forms_json) || []; } catch (err) { forms = []; }
        for (const form of forms) {
          if (!covered.has(form) && missing.indexOf(form) !== -1) {
            entries.push({ ...shape(row), key: form });
            covered.add(form);
          }
        }
      }
    } catch (err) {
      console.log('dictionary prefetch: forms pass skipped —', err.message);
    }
  }
