...
import { measures, sameMeasure, unsupportedMeasures } from '../../_lib/measures.js';
...
/* measures(), sameMeasure() and unsupportedMeasures() live in
   _lib/measures.js, shared with /api/knowledge/ask. They were duplicated here
   briefly and the two unit lists had already diverged — the shared one knows
   m/s and dBA, which this domain uses constantly and this copy did not. */
...
function engineeringPrompt({ product, factory, reference, kb, offering, comparison, decided, ctx, graph }) {
  const sources = [
    // Approved graph knowledge leads. It is the only source here that a person
    // signed off node by node, so where it and a learned table disagree, this
    // is the one that should shape the answer.
    graph,
    reference,
...
// unsupportedMeasures() now lives in _lib/measures.js, shared with
// /api/knowledge/ask. Two copies of this rule would drift, and the copy that
...
async function handlePost(context) {
  ...
  // The knowledge graph is read here, in process. Compliance Maker used to
  // ignore it entirely while the Knowledge section promised every visitor that
  // approved nodes are what it answers from.
  const projectId = String(body.projectId || '').trim() || null;

  const [facts, kb, options, criteria, graphResult] = await Promise.all([
    loadFacts(context, product, factory),
    loadKb(context, product),
    loadOptions(context, product),
    loadCriteria(context, product),
    retrieve(context.env, context.data && context.data.user, question, {
      projectId,
      limit: 8,
    }).catch((err) => {
      // A graph fault must not cost the engineer an answer the other sources
      // can still support.
      console.log('graph retrieval failed:', err && err.message);
      return null;
    }),
  ]);

  const graphText = knowledgeBlock(graphResult);
  ...
  const allowed = [question, reference, kbText, offering, comparison, ctx,
    // Values retrieved from the graph are on file by definition. Leaving them
    // out here would flag a correctly cited approved number as invented and
    // downgrade a good answer to TO VERIFY.
    allowedText(graphResult),
  ...
  const cited = citations(graphResult);
  context.waitUntil(logUsage(context.env, {
    result: graphResult,
    context: question,
    outcome: 'used',
    userId: context.data && context.data.user ? String(context.data.user.id) : null,
    answerId,
  }));
