...
import { requireUser, json, PRODUCTS, isValidFactory, answerLogTable, withErrorHandling, isMissingTable, SETUP_HINT } from '../../../_compliance.js';
...
async function handlePost(context) {
  ...
  try {
    ...
    // Logging is fire-and-forget from the client; losing it must not cost
    // the person their converted matrix.
    if (!isMissingTable(err)) throw err;
    return json({ saved: 0, notice: SETUP_HINT });
  }
...
async function handleGet(context) {
  ...
  try {
    ...
    // Here the export IS the feature, so this one does fail — but with the
    // sentence that says what to do, not the SQLite error.
    if (!isMissingTable(err)) throw err;
