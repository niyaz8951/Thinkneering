...
// Taken from a real completed matrix (589 rows, Al Bustan). What engineers
// actually write is NOT what the tool used to allow: "Comply with remarks"
// was the single most common status (41 uses), "Not Applicable" appeared 24
// times, and "Deviation" and "Not Comply" appeared ZERO times in the whole
// document. A vocabulary the reviewers do not use forces them to rewrite
// every row, which is worse than leaving the row blank.
//
//   Comply              meets it as standard
//   Comply with remarks meets it, but the offer differs from the wording —
//                       62 mm where 50 mm was asked. THE COMMON CASE.
//   Not Applicable      the clause does not apply to this selection
//   By Contractor       site execution, not equipment supply
//   Noted               acknowledged; nothing to comply with
//   TO VERIFY           nothing on file supports an answer
...
// A short framing sentence, NOT the whole catalogue. The previous version
// pasted every offering line into every call — 2,800 characters on a prompt
// that was already producing empty answers. The entries themselves now travel
// with the clause they concern, which is both smaller and more useful.
function kbPreamble(product, hasDatasheet, kb) {
  if (!kb.length) return '';
  return `\nWHAT WE OFFER: for each clause below, the ${product} standard offering is listed ` +
    `under "OUR STANDARD OFFERING". The first value is what we supply as standard; anything ` +
    `after it we can also supply. ` +
    (hasDatasheet
      ? `The attached datasheet describes the unit actually selected and overrides this — ` +
        `use the offering only for a property the datasheet does not state.\n`
      : `No datasheet is attached, so this is what the unit will be. Quote the standard value. ` +
        `If the standard does not meet the clause but a listed alternative does, answer ` +
        `"Comply with remarks" and name the alternative — we can supply it.\n`);
}

// ============================================================================
// DETERMINISTIC ANSWER — when the answer is already known, do not ask.
// ----------------------------------------------------------------------------
// Measured on a real run: every core product clause came back withdrawn —
// casings, insulation, drain pans, fan sections, motors — which are exactly
// the clauses the datasheet answers. The pipeline had already found "Panel •
// Insulation: 62 mm" and already computed "62 >= 50, thicker is better,
// satisfied". Then it asked an 8B model to phrase it, the model answered
// something unrelated, and the guard correctly deleted the answer.
//
// The answer was fully determined before the model was involved. So when a
// comparison resolves cleanly, the row is now answered in code and never
// sent to the model at all. Nothing generated means nothing to hallucinate,
// nothing to guard, and nothing to retry.
//
// Phrasing follows the house style observed in completed matrices:
//   "Offered units are with 62 mm foam insulation."
// ============================================================================
function deterministicAnswer(clauseText, relevant, criteria) {
  const req = measurements(clauseText);
  if (!req.length || !relevant.length) return null;

  // ONLY THE SINGLE BEST-MATCHING FIELD. Testing this on the real spec caught
  // two ways a looser rule goes wrong, both of which would have produced a
  // confident wrong answer with no model to blame and no guard to catch it:
  //
  //   - "Condensate drain pans insulated with minimum 16 mm" matched the
  //     PANEL insulation field and answered "62 mm foam insulation". Same
  //     unit, same word "insulation", completely different part.
  //   - "Casings shall be 50 mm thick" also matched "Panel Inner Skin:
  //     SS304 1.0 mm" — 1.0 mm against 50 mm reads as a failure and would
  //     have suppressed a genuinely compliant answer.
  //
  // So: one field, the best-scoring one, and it must be demonstrably about
  // the same thing as the clause.
  const f = relevant[0];
  // Expanded the same way the field matcher expands, plus a crude singular,
  // or "panels" in the clause fails to match "Panel" in the label and a
  // perfectly good answer is thrown away on a plural.
  // Stem FIRST, then expand: "skinned" has to become "skin" before the
  // synonym groups can turn it into "panel".
  const stemmed = new Set(words(clauseText));
  for (const w of [...stemmed]) {
    if (w.endsWith('ned')) stemmed.add(w.slice(0, -3));
    else if (w.endsWith('ed')) stemmed.add(w.slice(0, -2));
    if (w.endsWith('s')) stemmed.add(w.slice(0, -1));
  }
  const clauseWords = expandTerms([...stemmed]);

  // A distinctive word from the field's own label must appear in the clause.
  // Generic measurement words are excluded — "insulation" and "thickness"
  // are true of half the datasheet and prove nothing about the subject.
  const GENERIC = new Set(['insulation', 'insulated', 'thickness', 'thick', 'material',
                           'class', 'type', 'size', 'unit', 'supply', 'return', 'design']);
  const distinctive = words(String(f.label).replace(/\u2022/g, ' ')).filter(w => !GENERIC.has(w));
  if (!distinctive.length || !distinctive.some(w => clauseWords.has(w))) return null;

  // The clause and the field must resolve to the SAME comparison criterion,
  // so a pressure requirement is never answered with a thickness.
  const parts = [];
  for (const r of req) {
    for (const g of measurements(f.value).filter(x => x.unit === r.unit)) {
      const clauseCrit = matchCriterion(clauseText, criteria, r.unit);
      const fieldCrit = matchCriterion(f.label, criteria, r.unit);
      if (!clauseCrit || !fieldCrit || clauseCrit.name !== fieldCrit.name) return null;
      const v = compareValues(r.n, g.n, clauseCrit);
      if (!v) return null;                       // direction unknown — a human decides
      if (!v.ok) return null;                    // falls short — a commercial call, not ours
      const subject = String(f.label).split('\u2022').slice(-1)[0].trim().toLowerCase();
      const detail = String(f.value).replace(/\s*\u2022\s*/g, ' ').trim();
      parts.push(`${detail} ${subject}`.replace(/\s+/g, ' ').trim());
    }
  }
  if (!parts.length) return null;

  // House style, taken from completed matrices:
  //   "Offered units are with 62 mm foam insulation."
  return {
    status: 'Comply with remarks',
    remarks: 'Offered units are with ' + [...new Set(parts)].slice(0, 2).join(', ') + '.',
    verified: true,
    deterministic: true,
  };
}
      ...
      for (const g of measurements(f.value).filter(x => x.unit === r.unit)) {
        ...
        if (v) {
          lines.push(`  - Clause requires ${r.raw}. This unit is ${g.raw} (${f.label}), which ${v.why}. ` +
            `Correct status: ${v.ok ? 'Comply with remarks (state our value)' : 'TO VERIFY (our value does not meet it — that is a commercial call, not yours)'}. ` +
    ...
    for (const f of relevant) {
      for (const ac of classTokens(f.value)) {
        ...
        lines.push(`  - Clause requires class ${rc}. This unit is ${ac} (${f.label}), which ${v.why}. ` +
          `Correct status: ${v.ok ? 'Comply with remarks (state our class)' : 'TO VERIFY (our class does not meet it — a commercial call, not yours)'}. State ${ac}.`);
      }
    }
  }
...
// A withdrawal is NOT an answer, and it must never look like one.
//
// The reason text is written for the engineer reviewing the sheet — "AI
// repeated a sample answer instead of answering this clause" is a message
// about the tool, and it was going straight into the Remarks column that gets
// sent to a consultant. That is worse than the wrong answer it replaced.
//
// So: Remarks stays EMPTY (the row is genuinely unanswered), the reason goes
// to `comments`, which is the internal column deleted before sending, and
// `guarded` tells the client not to prefix it "[GUESS]" — nothing was
// guessed, the answer was removed.
function withdraw(reason) {
  return { status: 'TO VERIFY', remarks: '', comments: reason, verified: false, guarded: true };
}
...
function guardAnswer(suggestion, meta, sourceText) {
  ...
  // 0. The remark repeats the clause or its location instead of answering.
  //    Seen live: a clause answered with "PART 2 - PRODUCTS -> 2.1 AIR
  //    HANDLING UNITS", which is the section path supplied as CONTEXT. The
  //    model echoed its own input back as the answer.
  const meaningful = remark.replace(/[^a-z0-9]+/gi, ' ').trim().toLowerCase();
  const clauseWords = String((meta && meta.spec) || '').replace(/[^a-z0-9]+/gi, ' ').trim().toLowerCase();
  const pathWords = String((meta && meta.path) || '').replace(/[^a-z0-9]+/gi, ' ').trim().toLowerCase();
  if (meaningful && pathWords && (meaningful === pathWords || pathWords.includes(meaningful))) {
    return withdraw('AI returned the section heading instead of an answer.');
  }
  if (meaningful.length > 25 && clauseWords.includes(meaningful)) {
    return withdraw('AI repeated the clause back instead of answering it.');
  }
  ...
  if (meta && meta.verdicts && meta.verdicts.length) {
    ...
    // "Comply with remarks" is a COMPLY. Treating it as a negative here would
    // withdraw the single most common correct answer in a real matrix.
    const saysNo = /not comply|deviation|not applicable/.test(st);
...
- WHEN WHAT WE SUPPLY DIFFERS FROM THE WORDING BUT STILL MEETS THE INTENT, the status is
  "Comply with remarks" and the remark states WHAT IS ACTUALLY OFFERED. This is the most
  common answer in a real matrix, not an edge case. A clause asking for 50 mm panels answered
  from a 62 mm unit is: Comply with remarks | "Offered units are with 62 mm foam insulation."
  Never "Not Applicable" and never a bare "Comply" that hides the difference.
- "Not Applicable" is for a clause that does not apply to this selection at all — a belt-drive
  clause on a direct-drive unit, a steam coil clause on a chilled-water unit. State why in one
  short sentence.
- "Noted" is for a clause that asserts something rather than requiring it, where there is
...
- Datasheet-backed and matching: state the unit's real measured value.
    {"status":"Comply","remarks":"[actual value from the datasheet field] [what it is].","verified":true}
- Datasheet-backed but differing from the wording — THE COMMON CASE:
    {"status":"Comply with remarks","remarks":"Offered units are with [actual value] [what it is].","verified":true}
- Does not apply to this selection:
    {"status":"Not Applicable","remarks":"[why it does not apply to this unit].","verified":true}
...
async function handlePost(context) {
  ...
  // ---- knowledge, fetched ONCE and IN PARALLEL -------------------------
  // These were seven sequential round trips. For a 63-blank matrix that is
  // 21 batches x 7 queries = 147 serialised D1 calls, and the person watches
  // every one of them. They do not depend on each other, so they no longer
  // wait for each other.
  const clauseTexts = items.map(it => String(it.spec || '') + ' ' + String(it.path || ''));
  const [facts, profiles, unitCatalogue, specTopics, criteria, kb, vocab] = await Promise.all([
    loadFacts(context, product, factory),
    loadSectionProfiles(context, product, factory, items.map(it => it.path || '')),
    loadUnitSections(context, product),
    loadSpecTopics(context, product),
    loadCriteria(context, product),
    loadKb(context, product),
    loadTerms(context, product),
  ]);

  const reference = factsBlock(product, factory, pickFacts(clauseTexts, facts));
  const sectionBlock = unitSectionsBlock(product, unitSections, unitCatalogue);

  // Term discovery is a write and nothing waits on it.
  try {
    const seen = [];
  ...
  // Glossary, limited to terms this batch actually uses. A full glossary on
  // every call is the kind of prompt bloat that produces empty answers.
  const batchTerms = vocab.filter(t =>
    clauseTexts.some(c => c.toLowerCase().includes(String(t.term || '').toLowerCase())));
  const glossary = batchTerms.length
    ? 'GLOSSARY for terms in these clauses:\n' +
      batchTerms.slice(0, 12).map(t => `- ${t.term}: ${t.definition}`).join('\n') + '\n'
    : '';
  ...
  const clauseBlock = items.map((it, i) => {
    ...
    // What we offer for this clause. Present whether or not a datasheet is
    // attached: with one it fills the gaps, without one it IS the answer.
    const kbHits = kbForClause(spec + ' ' + path, kb);
    const kbBlock = kbHits.length
      ? `\n  OUR STANDARD OFFERING for this clause:\n${kbLines(kbHits)}`
      : '';
    // KB entries carry their own comparison direction, so "62 mm where 50 mm
    // was asked" is judged the same way whether the value came from the
    // datasheet or from the offering.
    const kbCompare = comparisonCheck(spec, kbHits.map(e => ({
      label: e.section + ' \u2022 ' + e.attribute, value: e.options[0] || '',
    })), kbHits.map(e => ({
      name: e.section + ' ' + e.attribute, terms: e.attribute + ',' + (e.aliases || ''),
      direction: e.direction, unit: e.unit, scale: e.scale,
    })).concat(criteria));
    ...
    clauseMeta[i] = {
      ...
      // Answered in code where the comparison resolves — see
      // deterministicAnswer(). These clauses skip the model entirely.
      settled: deterministicAnswer(spec, relevant.concat(kbHits.map(e => ({
        label: e.section + ' \u2022 ' + e.attribute, value: e.options[0] || '',
      }))), criteria),
    ...
    return `CLAUSE ${i + 1}: "${spec}"` +
      ...
      fieldLines + comparisonCheck(spec, relevant, criteria) + kbBlock + kbCompare +
  ...
  const userMsg = (datasheetBlock ? datasheetBlock + '\n' : '') +
                  ...
                  kbPreamble(product, hasDatasheet, kb) + glossary + clauseBlock;
  ...
  let guarded = 0;
  // The KB is an authorised source, so values quoted from it must not be
  // mistaken for fabrications by the traceability guard.
  const sourceText = kb.map(e => e.options.join(' ')).join(' ') + ' ' +
  ...
  const suggestions = attempt.suggestions.map((sg, i) => {
    // A settled clause outranks whatever the model said: it is derived from
    // the datasheet and the comparison rules, so the model cannot improve it
    // — and it needs no guard, because nothing was generated.
    const settled = clauseMeta[i] && clauseMeta[i].settled;
    if (settled) { settledCount++; return settled; }
  ...
  const out = { suggestions, model: MODEL, passes, guarded, notices, settled: settledCount };
