...
import { requireUser, json, PRODUCTS, isValidFactory, complianceTier, withErrorHandling } from '../../_compliance.js';
...
// Default factory configuration per product — authorized reference data
// supplied by the user. The selection report always overrides these in real
// projects; since this AI never sees selection reports, it may cite these
// only as "standard configuration" and must not invent beyond them.
const FACTORY_CONFIG = {
  'AHU':
    'DEFAULT DAIKIN KSA FACTORY CONFIGURATION (AHU) — authorized reference data:\n' +
  ...
  'FCU': '',                 // add verified FCU defaults here when available
  'Air Cooled Chiller': ''   // add verified chiller defaults here when available
};

function buildSystemPrompt(product, factory, itemCount, hasDatasheet) {
  const config = FACTORY_CONFIG[product];
  return (
...
Every clause gets an answer — do not leave this step with TO VERIFY as your first instinct. ${config ? 'You may cite the default factory configuration below as standard configuration (a project selection report would always override it; here you have none for this clause):\n' + config + '\n' : 'No default factory configuration is on file for this product.'}
...
async function handlePost(context) {
  ...
  try {
    const res = await context.env.AI.run(MODEL, {
      messages: [
        { role: 'system', content: buildSystemPrompt(product, factory, items.length, hasDatasheet) },
