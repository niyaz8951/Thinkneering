...
// The remark must CONTINUE the status, not restate it. The two columns are
// read together — "By Contractor | By Contractor / Others. Daikin scope is
// equipment supply only" wastes the first half of the remark saying what the
// cell to its left already says.
//
// Told not to, a small model does it anyway, so it is also removed here. The
// prompt asks; this guarantees.
function trimStatusEcho(status, remark) {
  let out = String(remark || '').trim();
  if (!out) return out;

  // "Comply with remarks | Offered units are..." — the whole row emitted as
  // one string, with the status pasted in front of its own remark.
  const piped = out.match(/^\s*([A-Za-z][A-Za-z ]{2,25})\s*[|]\s*(.+)$/s);
  if (piped && STATUSES.some(st => st.toLowerCase() === piped[1].trim().toLowerCase())) {
    out = piped[2].trim();
  }

  // A leading copy of the status, with or without the "/ Others" tail, and
  // whatever punctuation joins it to the real sentence.
  const st = String(status || '').trim();
  if (st) {
    const esc = st.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    out = out.replace(new RegExp('^\\s*' + esc + '\\s*(?:/\\s*Others)?\\s*[.:,;–—-]*\\s*', 'i'), '');
  }

  // Left with nothing but the status word, there was never a remark.
  if (!out || STATUSES.some(x => x.toLowerCase() === out.toLowerCase().replace(/[.\s]+$/, ''))) return '';
...
function guardAnswer(suggestion, meta, sourceText) {
  ...
  // Strip the status echo before anything else looks at the remark, so the
  // guards below judge the actual answer rather than its prefix.
  suggestion = Object.assign({}, suggestion, {
    remarks: trimStatusEcho(suggestion.status, suggestion.remarks),
  });
  if (!suggestion.remarks) return suggestion;
...
- By Contractor: any site-execution requirement (installation, erection, rigging, lifting, unloading, storage, positioning, alignment, anchoring, assembly, duct connection, piping, electrical connection, touch-up painting, site testing, commissioning labour). Remark must be VERBATIM, word for word, nothing added before or after: "Daikin scope is equipment supply only." Never classify these as Comply, Deviation, or Not Comply.
...
- INTERNAL NOTES attached to past answers are your team's own guidance: why an answer is worded
  the way it is, what a certificate actually covers, what to be careful of. READ THEM BEFORE
  ANSWERING and let them shape the answer. They are internal — never quote one, never mention
  that a note exists, and never copy its wording into a remark a consultant will read.
- THE REMARK MUST NOT REPEAT THE STATUS. The two columns are read together, so the remark
  CONTINUES the status rather than restating it.
    RIGHT:  status "By Contractor"        remark "Daikin scope is equipment supply only."
    WRONG:  status "By Contractor"        remark "By Contractor / Others. Daikin scope is..."
    RIGHT:  status "Comply with remarks"  remark "Offered units are with 62 mm foam insulation."
    WRONG:  status "Comply with remarks"  remark "Comply with remarks | Offered units are..."
  Never begin a remark with the status word, and never put the status and the remark in one
  string separated by a pipe or a dash.
...
- Site work: this one wording is fixed and is the only text you may reproduce exactly.
    {"status":"By Contractor","remarks":"Daikin scope is equipment supply only.","verified":true}
  ...
  const clauseBlock = items.map((it, i) => {
    ...
    const ctx = (Array.isArray(it.context) ? it.context : []).slice(0, 5)
      .map(c => `    - Past clause: "${String(c.spec || '').slice(0, 300)}"\n` +
                ...
                // The team's own note on that answer: why it is phrased that
                // way, what the certificate really covers, what to watch. It
                // is internal — it explains the answer, it is never quoted.
                (c.note ? `\n      Internal note (guidance, NEVER quote it): "${String(c.note).slice(0, 300)}"` : ''))
      .join('\n');
