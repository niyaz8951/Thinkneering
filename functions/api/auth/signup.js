...
export const onRequestPost = async ({ env, request }) => {
  ...
  // Fails closed. If the settings row is ever missing — a fresh database, a
  // bad restore, a typo in the key — the old fallback of 'open' handed out
  // live accounts silently. 'approval' means the worst case is an admin
  // having to press Approve, not a stranger already inside.
  const mode = await setting(env, 'signup_mode', 'approval');
