 ...
 *           { type: 'node', id, positionOnly: true, x, y, lane? }  layout only,
 *           no version bump and no change of approval status
...
async function saveNode(env, user, body) {
  ...
  // by rewriting x/y, and the graph drag could too; neither should bump the
  // version, write a revision, or pull an approved node out of the index.
  // Only what a node SAYS is knowledge; where it sits on the canvas is not.
