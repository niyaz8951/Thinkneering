...
import {
  ...
  rowToNode, rowToEdge, withJson
} from '../../_lib/knowledge.js';
import { refreshCounts } from './maps.js';
import { visibleMapIds } from '../../_lib/graph-retrieval.js';
...
async function _onRequestGet(context) {
  ...
  /* No map asked for means "which maps can I open". The desk needs this to
     show a count per map, and it was a 400 — so the only way to find out
     what you had access to was to already know a map id. */
  if (!mapId) {
    const ids = await visibleMapIds(env, user);
    if (!ids.length) return json({ ok: true, maps: [] });
    const ph = ids.map(() => '?').join(',');
    const rows = await env.DB.prepare(
      'SELECT id, slug, title, description, domain, node_count, approved_count, updated_at ' +
      'FROM knowledge_maps WHERE id IN (' + ph + ') ORDER BY title'
    ).bind(...ids).all();
    return json({ ok: true, maps: (rows && rows.results) || [] });
  }
...
async function saveNode(env, user, body) {
  ...
  // Position is layout, not knowledge. The outline reorders and reparents
  // by rewriting x/y; that must not bump the version, write a revision, or
  // pull an approved node out of the index. What a node SAYS is knowledge;
  // where it sits is not.
  if (body.positionOnly) {
    if (!body.id) return { error: 'A position needs an existing node' };
    const owns = await env.DB.prepare(
      'SELECT id FROM knowledge_nodes WHERE id = ? AND map_id = ?'
    ).bind(body.id, body.mapId).first();
    if (!owns) return { error: 'Node not found', status: 404 };
    await env.DB.prepare(
      'UPDATE knowledge_nodes SET x = ?, y = ?, lane = COALESCE(?, lane), updated_by = ?, updated_at = ? WHERE id = ?'
    ).bind(
      Number(body.x) || 0, Number(body.y) || 0,
      body.lane === undefined ? null : String(body.lane || '').slice(0, 60),
      uid, now, body.id
    ).run();
    return { id: body.id, status: 'position-saved' };
  }

  if (!body.title || !String(body.title).trim()) return { error: 'A node needs a title' };
  if (!body.kind) return { error: 'A node needs a kind' };

  // Scope is validated rather than trusted: 'project' is the safe default
  // because it is the claim that commits to least. A value that turns out to
  // hold generally can be promoted; one wrongly filed as general has already
  // been quoted as a standing rule by the time anyone notices.
  const SCOPES = ['project', 'family', 'general'];
  const scope = SCOPES.indexOf(String(body.scope || '')) !== -1 ? String(body.scope) : null;
  ...
  if (body.id) {
    ...
    await env.DB.prepare(
      ...
      'tags=?, standards=?, lane=?, x=?, y=?, scope=?, notes=?, ai_open=?, status=?, version=version+1, ' +
  ...
  await env.DB.prepare(
    ...
    'tags, standards, lane, x, y, scope, status, created_by, created_at, updated_at) ' +
    'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
  ).bind(
    id, body.mapId, fields.kind, fields.title, fields.aliases, fields.summary, fields.body,
    fields.attributes, fields.tags, fields.standards, fields.lane, fields.x, fields.y,
    scope || 'project', 'draft', uid, now, now
  ).run();
