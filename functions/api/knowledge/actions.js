...
export const onRequestGet = withJson(async (context) => {
  ...
  /* No mapId means "everything I can see". The desk asks this: what is on my
     plate is a question about a person, not about one map, and making the
     caller loop over maps would put the same join in three places. */
  if (!mapId) {
    const maps = await visibleMapIds(env, user);
    if (!maps.length) return json({ ok: true, actions: [], open: 0, overdue: 0 });

    const ph = maps.map(() => '?').join(',');
    const res = await env.DB.prepare(
      'SELECT * FROM knowledge_open_actions WHERE map_id IN (' + ph + ') ' +
      'ORDER BY overdue DESC, priority ASC, due IS NULL, due ASC, created_at ASC LIMIT 50'
    ).bind(...maps).all();
