...
MIGRATIONS = [
    'db/schema.sql',
    'db/2026-09-migration-ledger.sql',
    ...
    'db/2026-09-rename-sections.sql',
    'db/2026-09-domain-kinds.sql',
    'db/2026-09-approve-seed.sql',
    'db/2026-09-outline.sql',
    'db/2026-09-dictionary-corrections.sql',
]

# Mirrors db/add-columns.sh. These live outside the migration files because a
# failing ALTER aborts the rest of a D1 file; the script runs each as its own
# call. Applying them here in the same order keeps the tested schema identical
# to the deployed one.
COLUMNS = [
...
# The column each migration needs must exist before that migration runs.
COLUMNS_AFTER = {
...
for m in MIGRATIONS:
    ...
    for table, column in COLUMNS_AFTER.get(m, []):
        try:
            con.execute("ALTER TABLE %s ADD COLUMN %s" % (table, column))
        except sqlite3.OperationalError as e:
            # "duplicate column name" is the expected result on a re-add, and
            # is exactly what the deploy script tolerates.
...
check('site is exactly the three sections, renamed for clarity',
      lambda: (_ for _ in ()).throw(AssertionError(secs))
      if secs != ['compliance-maker', 'maps', 'library'] else None)
...
# The rule changed deliberately, and got more precise. "Nothing pre-approved"
# was right for claims and wrong for structure: making someone click fifty
# times to confirm their own process is ceremony, and until they finish, the
# ask box returns nothing and the system looks broken.
#
# What is approved is a vocabulary. What is never pre-approved is a fact.
seeded_facts_approved = con.execute(
    "SELECT COUNT(*) FROM knowledge_facts f JOIN knowledge_nodes n ON n.id = f.node_id "
    "WHERE n.created_by = 'seed' AND f.status = 'approved'").fetchone()[0]
check('no seeded FACT is ever pre-approved — a claim still needs a person',
      lambda: (_ for _ in ()).throw(AssertionError(seeded_facts_approved))
      if seeded_facts_approved else None)
...
# --- the seed is usable on day one
seeded = con.execute("SELECT COUNT(*) FROM knowledge_nodes WHERE created_by='seed'").fetchone()[0]
seed_draft = con.execute(
    "SELECT COUNT(*) FROM knowledge_nodes WHERE created_by='seed' AND status='draft'").fetchone()[0]
check('the seeded structure is approved, so the system answers immediately',
      lambda: (_ for _ in ()).throw(AssertionError('%d still draft' % seed_draft)) if seed_draft else None)

indexed = con.execute(
    "SELECT COUNT(*) FROM knowledge_terms t JOIN knowledge_nodes n ON n.id=t.node_id "
    "WHERE n.created_by='seed'").fetchone()[0]
check('and indexed, or it would be approved and still unfindable',
      lambda: (_ for _ in ()).throw(AssertionError(indexed)) if indexed < seeded else None)

# Approving structure is only safe because the seed asserts no facts.
seed_facts = con.execute(
    "SELECT COUNT(*) FROM knowledge_facts f JOIN knowledge_nodes n ON n.id=f.node_id "
    "WHERE n.created_by='seed'").fetchone()[0]
check('the seed contains no facts — which is what makes approving it safe',
      lambda: (_ for _ in ()).throw(AssertionError(seed_facts)) if seed_facts else None)

# A hand-authored draft must not be swept up by the same migration.
mine = str(uuid.uuid4())
con.execute("INSERT INTO knowledge_nodes (id,map_id,kind,title,status,created_by,created_at,updated_at)"
            " VALUES (?,'map_sbu','note','My draft','draft','u1',datetime('now'),datetime('now'))",
            (mine,))
con.executescript(open(os.path.join(ROOT, 'db/2026-09-approve-seed.sql')).read())
still = con.execute("SELECT status FROM knowledge_nodes WHERE id=?", (mine,)).fetchone()[0]
check('it cannot approve work you authored by hand',
      lambda: (_ for _ in ()).throw(AssertionError(still)) if still != 'draft' else None)

terms_once = con.execute("SELECT COUNT(*) FROM knowledge_terms").fetchone()[0]
con.executescript(open(os.path.join(ROOT, 'db/2026-09-approve-seed.sql')).read())
terms_twice = con.execute("SELECT COUNT(*) FROM knowledge_terms").fetchone()[0]
check('re-running does not duplicate the index',
      lambda: (_ for _ in ()).throw(AssertionError((terms_once, terms_twice)))
      if terms_once != terms_twice else None)

# ── The outline relation ─────────────────────────────────────────────
# `under` is the one edge the outline can always draw: legal between every
# pair of registered kinds, including ones the physical hierarchy forbids.
kinds = [r[0] for r in con.execute("SELECT kind FROM knowledge_kinds WHERE is_active=1")]
pairs = con.execute("SELECT COUNT(*) FROM knowledge_edge_rules WHERE relation='under'").fetchone()[0]
check('under is legal for every pair of kinds',
      lambda: (_ for _ in ()).throw(AssertionError((pairs, len(kinds)))) if pairs < len(kinds) * len(kinds) else None)

def outline_edge():
    a, b = nid(), nid()
    con.execute("INSERT INTO knowledge_nodes (id,map_id,kind,title,status,created_by,created_at,updated_at)"
                " VALUES (?,'map_sbu','clause','A clause','draft','u1',datetime('now'),datetime('now'))", (a,))
    con.execute("INSERT INTO knowledge_nodes (id,map_id,kind,title,status,created_by,created_at,updated_at)"
                " VALUES (?,'map_sbu','project','A project','draft','u1',datetime('now'),datetime('now'))", (b,))
    con.execute("INSERT INTO knowledge_edges (id,map_id,from_id,to_id,relation,status,created_by,created_at)"
                " VALUES (?,'map_sbu',?,?,'under','draft','u1',datetime('now'))", (nid(), a, b))
    rejected("INSERT INTO knowledge_edges (id,map_id,from_id,to_id,relation,status,created_by,created_at)"
             " VALUES (?,'map_sbu',?,?,'contains','draft','u1',datetime('now'))", (nid(), a, b))
check('a project can sit under a clause in the outline, but a clause still cannot contain one', outline_edge)

re_rules = con.execute("SELECT COUNT(*) FROM knowledge_edge_rules").fetchone()[0]
con.executescript(open(os.path.join(ROOT, 'db/2026-09-outline.sql')).read())
re_rules2 = con.execute("SELECT COUNT(*) FROM knowledge_edge_rules").fetchone()[0]
