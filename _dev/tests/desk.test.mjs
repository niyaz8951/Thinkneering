...
t('the footer points at the new slugs',
  g.includes("sectionHref('maps')") && g.includes("sectionHref('library')"));
// Sections link straight to their homes now: the /s/<slug> hop (section.html,
// a catalogue call, then location.replace) is what went nowhere on a phone.
t('a section link resolves to the tool, not to the /s/ hop',
  /maps: '\/tools\/knowledge\/'/.test(g) && /library: '\/education\/'/.test(g));
t('the tab bar uses the same resolution', /sectionHref\(s\.slug\)[\s\S]{0,200}inSection\(s\.slug\)/.test(g));
t('no navigation still points at the old slugs',
  !/href=\\"\/s\/knowledge|href=\\"\/s\/education/.test(g));

// --- the desk has to carry work, not just a search box
t('your maps are listed with their counts', home.includes('function loadMaps'));
t('each map links straight to its graph', home.includes('/tools/knowledge/map.html?map='));
t('the top of the review queue is shown in full, not just counted',
  home.includes('function loadNext') && home.includes('desk__nextcard'));
t('it names what is actually proposed on that node',
  home.includes('proposed fact'));
t('and warns when something similar is already held',
  home.includes('similar already held'));
t('all three load on sign-in', /loadQueues\(\);[\s\S]{0,80}loadMaps\(\);[\s\S]{0,40}loadNext\(\)/.test(home));
t('a missing section fails quietly rather than blanking the desk',
  (home.match(/catch \(e\) \{ \/\* no/g) || []).length >= 3);

// /api/knowledge/graph used to 400 without a map id, so the only way to learn
// which maps you could open was to already know one.
const graph = readFileSync(resolve(ROOT, 'functions/api/knowledge/graph.js'), 'utf8');
t('the graph API lists accessible maps when no map is named',
  graph.includes('which maps can I open'));
t('and reuses the one implementation of who may read what',
  graph.includes("visibleMapIds") && graph.includes("_lib/graph-retrieval.js"));
