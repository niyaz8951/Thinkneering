...
// The whole tile is a stretched link, so a footer control that is not lifted
// above the overlay renders perfectly and does nothing when pressed. Shelve
// was dead for exactly this reason.
const edu = readFileSync(resolve(ROOT, 'assets/css/education.css'), 'utf8');
t('the tile really is a stretched link',
  /\.book-card__open::after\s*{[^}]*position:\s*absolute[^}]*inset:\s*0/.test(edu));
t('every footer button is lifted above it, not just the ones named so far',
  /\.book-card\s+\.card__foot\s+\.btn\s*{[^}]*z-index:\s*1/.test(edu));
t('and is positioned, or z-index does nothing',
  /\.book-card\s+\.card__foot\s+\.btn\s*{[^}]*position:\s*relative/.test(edu));
t('Remove keeps its right alignment',
  /\.book-card__remove\s*{[^}]*margin-left:\s*auto/.test(edu));
t('the stylesheet version was bumped so the fix is not cached out',
  !page.includes('education.css?v=3'));
