...
t('it keeps no store of its own', !/localStorage[\s\S]{0,80}outline|outline[\s\S]{0,80}localStorage/i.test(js));
...
t('a write is followed by a reload of both views', /async function commitTitle[\s\S]{0,900}await reload\(\)/.test(js) &&
  /async function reparent[\s\S]{0,1600}await reload\(\)/.test(js));

// --- the tree is an edge
t('indentation is a relation the pack names', js.includes('pack.outline.relation'));
t('Tab creates that edge from the new parent', /async function reparent[\s\S]{0,400}postEdge\(newParentId, n\.id\)/.test(js));
t('and removes the old one first', /async function reparent[\s\S]{0,300}deleteEdge\(old\.id\)/.test(js));
...
t('reparenting uses it', /async function reparent[\s\S]{0,1400}savePosition\(n\)/.test(js));
...
t('keeping the children moves them up', /mode === 'branch'[\s\S]{0,700}postEdge\(grand, kids\[k\]\.id\)/.test(js));
